...
# ======================== rolling avg by 5 frames =============================

tracking_avg <- tracking_bc |>
  arrange(gameId, playId, nflId, frameId) |> 
  group_by(gameId, playId, nflId, displayName) |> 
  #dividing frames in groups of 5:
  mutate(frame_group = (row_number() - 1) %/% 5 + 1) |> 
  group_by(gameId, playId, nflId, displayName, frame_group) |> 
  summarize(
    #avg speed across five frames
    speed_5 = mean(s_mph, na.rm = TRUE),                 
    #avg dir acc across five frames
    dir_a_mpsh_5 = mean(dir_a_right_mpsh, na.rm = TRUE) 
  ) |> 
  ungroup()


#====================== new QGAM model with rolling avg =========================

eff_function_qgam <- function(name, graph = FALSE) {
  # Choosing player name
  player_runs <- tracking_avg |> 
    filter(displayName == name)
  
  # Making the modeling data frame
  player_runs_modeling <- player_runs |> 
    select(speed_5, dir_a_mpsh_5, gameId, displayName, bc_id, playId)
  ...
  qgam_fit_a_top <- qgam(dir_a_mpsh_5 ~ s(speed_5, k = 10, bs = "ad"),
                         data = data_pos,
                         qu = .95,
                         multicore = TRUE,
                         ncores = 7)
  
  qgam_fit_a_bottom <- qgam(dir_a_mpsh_5 ~ s(speed_5, k = 10, bs = "ad"),
                            data = data_neg,
                            qu = .05,
                            multicore = TRUE,
                            ncores = 7)
  ...
  if (graph == TRUE) {
    ...
    player_graph <- player_runs_test_preds |> 
      ggplot(aes(x = speed_5, y = dir_a_mpsh_5)) +
      geom_point(alpha=.6, color="grey2")+
      geom_point(data = out_line, aes(x = speed_5, y = dir_a_mpsh_5, fill = "Adjusted distance = 0"), size = 4, 
