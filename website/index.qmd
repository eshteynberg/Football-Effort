...
### Adding more players to the dropdown menu:

```{r}
#| label: intercative-quant-regression
#| fig-cap: "Percentage of total points that lie in between the percentile $P_{99}$ and $P_{99}-3$ for each running back"
#| message: false
#| warning: false

# Player list
rb_dd_list <- c("Chase Edmonds", "Rachaad White", "Alexander Mattison", 
                "Najee Harris", "Matt Breida", "Melvin Gordon", "James Cook",
                "D'Andre Swift", "Joshua Kelley", "Latavius Murray", "Breece Hall",
                "Brian Robinson", "David Montgomery")

# Step 1: Initialize plot and storage
fig2 <- plot_ly()
all_traces <- list()
visibility_masks <- list()
trace_lengths <- c()

# Step 2: Loop through each player and collect their traces
for (i in seq_along(rb_dd_list)) {
  ...
  for (trace in plot_obj$x$data) {
    trace$visible <- (i == 1)  # Show only the first player's traces by default
    fig2 <- fig2 %>% add_trace(
      x = trace$x,
      y = trace$y,
      type = trace$type,
...
# Step 3: Create visibility masks for dropdown
