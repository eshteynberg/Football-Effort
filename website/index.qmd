...
# Step 3: Initialize new figure
fig <- plot_ly()
# Step 4: Extract and add only the visible trace(s)
...
# Step 5: Create visibility vectors
n1 <- length(g1$x$data)
n2 <- length(g2$x$data)

vis_sine <- c(rep(TRUE, n1), rep(FALSE, n2))
vis_cos  <- c(rep(FALSE, n1), rep(TRUE, n2))
# Step 6: Add dropdown menu
fig <- fig %>% layout(
  title = "Toggle between ggplotly Sine and Cosine",
  updatemenus = list(
    list(
      type = "dropdown",
      y = 0.8,
