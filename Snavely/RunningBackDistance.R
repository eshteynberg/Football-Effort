...
saquon <- tracking_rb_runs |> 
  filter(playId == 1948,
         gameId == 2022091108) |> 
  mutate(team_col = case_when(club == "TEN" ~ "#a5acaf",
                              club == "NYG" & displayName != "Saquon Barkley" ~ "#0B2265",
                              club == "football" ~ "#964B00",
                              displayName == "Saquon Barkley" ~ "orange"),
...
a <- field_background +
  ...
  labs(title = "<span style='color: #a5acaf;'> <br> New York Giants</span> @ <span style='color: #4B92DB;'>Tennessee Titans</span>, <br>week 1 of the 2022 NFL season",
...
a_gif <- animate(a, height = 600, width = 800)
a_gif
...
b <- just_saquon |> 
  ...
  geom_line(lwd = 2) +
  scale_color_manual("Metric", values = c("#0B2265", "#a5acaf")) +
  labs(x = "Frame number since snap", 
       y = "Joules",
       title = "<span style='color: orange;'>S. Barkley</span> kinetic energy and work throughout play") +
...
b_gif <- animate(b, height = 300, width = 800)
b_gif
